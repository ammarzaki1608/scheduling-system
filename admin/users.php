...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    $name = trim($_POST['user_name'] ?? ''); $email = trim($_POST['email'] ?? ''); $password = trim($_POST['password'] ?? ''); $role = trim($_POST['role'] ?? 'agent'); $pod_id = filter_input(INPUT_POST, 'pod_id', FILTER_VALIDATE_INT); $user_id = filter_input(INPUT_POST, 'user_id', FILTER_VALIDATE_INT);
    ...
    if (empty($errors)) {
        $stmt_check_email = $mysqli->prepare("SELECT User_ID FROM Users WHERE Email = ? AND User_ID != ?"); $check_id = $user_id ?? 0; $stmt_check_email->bind_param("si", $email, $check_id); $stmt_check_email->execute(); $stmt_check_email->store_result();
        if ($stmt_check_email->num_rows > 0) { $errors[] = "This email address is already in use by another account."; } $stmt_check_email->close();
    }
    if (empty($errors)) {
        $pod_value = ($role === 'admin') ? NULL : $pod_id;
        if ($_POST['action'] === 'edit_user' && $user_id) {
            if (!empty($password)) { $hashed_password = password_hash($password, PASSWORD_DEFAULT); $stmt = $mysqli->prepare("UPDATE Users SET User_Name=?, Email=?, Role=?, Password=?, Pod_ID=? WHERE User_ID=?"); $stmt->bind_param("ssssii", $name, $email, $role, $hashed_password, $pod_value, $user_id); } else { $stmt = $mysqli->prepare("UPDATE Users SET User_Name=?, Email=?, Role=?, Pod_ID=? WHERE User_ID=?"); $stmt->bind_param("sssii", $name, $email, $role, $pod_value, $user_id); }
            if ($stmt->execute()) { $success = "User updated successfully!"; } else { $errors[] = "Failed to update user."; } $stmt->close();
        } elseif ($_POST['action'] === 'add_user') {
            $hashed_password = password_hash($password, PASSWORD_DEFAULT); $stmt = $mysqli->prepare("INSERT INTO Users (User_Name, Email, Role, Password, Pod_ID, Created_At) VALUES (?, ?, ?, ?, ?, NOW())"); $stmt->bind_param("ssssi", $name, $email, $role, $hashed_password, $pod_value);
            if ($stmt->execute()) { $success = "User added successfully!"; } else { $errors[] = "Failed to add user."; } $stmt->close();
        }
    }
}
if (isset($_GET['action']) && $_GET['action'] === 'delete' && isset($_GET['id'])) { $user_id_to_delete = (int)$_GET['id']; $stmt = $mysqli->prepare("DELETE FROM Users WHERE User_ID = ?"); $stmt->bind_param("i", $user_id_to_delete); $stmt->execute(); $stmt->close(); header("Location: " . BASE_URL . "admin/users.php?deleted=1"); exit; }
...
$users_sql = "SELECT u.User_ID, u.User_Name, u.Email, u.Role, u.Pod_ID, p.Pod_Name, t.Team_Name FROM Users u LEFT JOIN Pods p ON u.Pod_ID = p.Pod_ID LEFT JOIN Teams t ON p.Team_ID = t.Team_ID ORDER BY u.User_Name ASC";
                ...
                <div class="modal-body">
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-6">
                            ...
                            <!-- The feedback div has been removed -->
                        </div>
                        <div class="col-md-6">
...
<?php if (!empty($success)): ?>
    <div class="alert alert-success alert-dismissible fade show" role="alert"><?= htmlspecialchars($success) ?><button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button></div>
<?php endif; ?>
<?php if (!empty($errors)): ?>
        ...
        <div class="table-responsive">
            <table class="table table-hover align-middle">
                <thead class="table-light">
                    <tr><th>Name</th><th>Email</th><th>Role</th><th>Team / Pod</th><th>Actions</th></tr>
                </thead>
                <tbody>
                    <?php foreach ($users as $user): ?>
                        <tr>
                            ...
                            <td><?php if ($user['Role'] === 'agent'): ?><?= htmlspecialchars($user['Team_Name'] ?? 'N/A') ?> / <?= htmlspecialchars($user['Pod_Name'] ?? 'N/A') ?><?php else: ?><span class="text-muted">N/A</span><?php endif; ?></td>
                            <td>
                                <button class="btn btn-sm btn-outline-secondary edit-btn" data-bs-toggle="modal" data-bs-target="#userModal" data-user='<?= htmlspecialchars(json_encode($user), ENT_QUOTES, 'UTF-8') ?>'><i class="bi bi-pencil-fill"></i></button>
                                <a href="?action=delete&id=<?= $user['User_ID'] ?>" class="btn btn-sm btn-outline-danger" onclick="return confirm('Are you sure you want to delete this user?');"><i class="bi bi-trash-fill"></i></a>
                            </td>
                        </tr>
                    <?php endforeach; ?>
                </tbody>
